...
import org.example.MySQLTest.{MyGraph, MySQLRunner}
...
object MySQLTest {
  ...
  val MySQLRunner: DemoRunner = new DemoRunner(MyGraph)
}
...
class MySQLTest {
  ...
  def IS1(): Unit = {
    ...
    val p = Map("personId" -> "300000000008100", "placeId" -> "200000000000981")
//    run(q, p)
...
//    }
  ...
  def u2(): Unit = {
    ...
    val result = MySQLRunner.run(verify, p)
    Assert.assertTrue(
      result.records()
