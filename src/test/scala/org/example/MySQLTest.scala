...
import org.example.MySQLTest.MySQLTestBase
...
object MySQLTest {
  val MySQLTestBase: MySQLTestBase = new MySQLTestBase

  @BeforeClass
  def importData(): Unit ={
    Profiler.timing("Import the test data. ", MySQLTestBase.loadMySQL())
  }
}
...
class MySQLTest {
  ...
  def run(cypher: String, params: Map[String, Any]): Unit = {
    try {
      val r = MySQLTestBase.run(cypher, params)
    } catch {
  ...
  def IS1(): Unit = {
    ...
    //val p = Map("personId" -> "300000000000014")
    val p = Map("firstName" -> "Hossein")
//    run(q, p)
    val result = MySQLTestBase.run(q, p)
      // .map(_.getAsString("cityId"))
    result.show()
//    while(result.hasNext) {
//      val element = result.next()
//      println(element)
//    }
  }
  ...
  def u2(): Unit = {
    ...
    val result = MySQLTestBase.run(verify, p)
    Assert.assertTrue(
      result.records()
